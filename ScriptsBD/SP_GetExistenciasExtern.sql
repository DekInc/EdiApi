...
GO
CREATE PROCEDURE [dbo].[SP_GetExistenciasExtern] 
@IdClient int 
AS 
BEGIN
SELECT Inv.Cliente,
     ...
     (SELECT T.TransaccionID,
      ...
      LEFT OUTER JOIN
        (SELECT Sy.InventarioID,
         ...
         INNER JOIN wms_test_29_01_2019.dbo.Pedido AS Pe WITH(NOLOCK) ON Pe.PedidoID = Sy.PedidoID AND Pe.clienteID = @IdClient
         WHERE (Pe.EstatusID NOT IN (9,
                                     10))
         GROUP BY Sy.InventarioID,
                  Sy.ItemInventarioID,
                  Sy.CodProducto,
                  sy.lote) AS Sy_1 ON Sy_1.InventarioID = It.InventarioID
      ...
      WHERE (T.IDTipoTransaccion = 'IN')
        ...
        AND T.EstatusID>4) AS Ie
   INNER JOIN wms_test_29_01_2019.dbo.Clientes AS C WITH(NOLOCK) ON C.ClienteID = Ie.ClienteID AND C.ClienteID = @IdClient
...
LEFT OUTER JOIN
  (SELECT C.Nombre AS Cliente,
   ...
   INNER JOIN wms_test_29_01_2019.dbo.Clientes AS C WITH(NOLOCK) ON C.ClienteID = P.ClienteID AND C.ClienteID = @IdClient
   INNER JOIN wms_test_29_01_2019.dbo.Producto AS Pd WITH(NOLOCK) ON Pd.CodProducto = Dp.CodProducto
   WHERE (P.EstatusID = 7
          OR P.EstatusID = 8)
     AND (Dp.PedidoID NOT IN
   ...
   GROUP BY C.Nombre,
            ...
            B.BodegaID,
            Dp.uxb,
            Dp.lote) AS Pedidos ON Pedidos.CodProducto = Inv.CodProducto
