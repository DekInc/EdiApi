...
BEGIN
	...
	--BEGIN
		...
		INNER JOIN wms.dbo.DtllPedido AS Dp WITH (NOLOCK) 
			...
			AND Dp.CodProducto like @TiendaId + '%'		
		WHERE ClienteID = @ClienteId
		AND P.PedidoID NOT IN (SELECT P2.PedidoWMS FROM EdiDB.dbo.PedidosExternos P2 WITH(NOLOCK) WHERE P2.PedidoWMS IS NOT NULL)
		ORDER BY P.fechapedido DESC
	--END
END
GO

--EXEC SP_GetPedidosMWmsByTienda 1432, '7384'
